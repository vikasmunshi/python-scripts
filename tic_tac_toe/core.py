...
@cached
@remembered
def check_winner(board: Board, one: Player, two: Player) -> str:
    return decided(one, two) if last_move_has_won(board) else draw(one, two) if board_is_full(board) else ''
...
def play_match_eliminate(size: int, num_double_games: int, one: Player, two: Player, round_num: int) -> Players:
    results = count_sub_items(play_game_set(size, one, two) for _ in range(num_double_games))
    wins_one, wins_two = results.get(decided(one, two), 0), results.get(decided(two, one), 0)
    invalid_one = results.get(invalid(one, two), 0)
    invalid_two = results.get(invalid(two, one), 0)
...
def play_match_points(size: int, num_double_games: int, one: Player, two: Player) -> Scores:
    results = count_sub_items(play_game_set(size, one, two) for _ in range(num_double_games))
    wins_one, wins_two = results.get(decided(one, two), 0), results.get(decided(two, one), 0)
    draws = results.get(draw(one, two), 0) + results.get(draw(two, one), 0)
    invalid_one = results.get(invalid(one, two), 0)
    invalid_two = results.get(invalid(two, one), 0)
...
def play_tournament_points(size: int, num_games: int, players: Players) -> Scores:
    results = flatten(play_match_points(size, num_games // 2, one, two)
...
def play_tournament_eliminate(size: int, num_games: int, players: Players, round_num: int, rounds: int = 9) -> Players:
    if len(players) <= 1 or round_num > 9:
        ...
        eliminated = flatten(play_match_eliminate(size, num_games // 2, one, two, round_num)
                             for n, one in enumerate(players) for two in players[n + 1:])
